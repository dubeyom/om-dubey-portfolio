...
const Navbar = () => {
    ...
    return (
        <div className='z-50 fixed flex justify-center w-full text-white font-bold'>
            ...
            <div className='border border-white/20 mt-8 backdrop-blur-3xl rounded-3xl hidden md:flex items-center justify-center p-2 max-w-[400px] mx-auto'>
                <ul className='flex flex-row p-2 space-x-8'>
                    {navLinks.map((link, index) => (
                        <li key={index}>
                            <Link href={link.path}
                                className='transform hover:text-white/50 transition-all duration-300 rase-in-out'
                            >
                                {link.title}
                            </Link>
            ...
            <div onClick={toggleNav} className='md:hidden absolute top-5 right-14 border rounded z-200 text-white/70 border-white/70 p-2'>
